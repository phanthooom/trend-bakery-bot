...
export default function CartPage() {
  ...
  const handleOrder = () => {
    if (!address) { navigate('/map'); return }
    navigate('/checkout')
  }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 pb-48">
      <div className="px-4 pb-3 bg-white border-b border-gray-100" style={{ paddingTop: `max(${safeTop + 16}px, calc(env(safe-area-inset-top) + 16px))` }}>
        <h1 className="font-bold text-xl text-gray-900">{t('cart')}</h1>
      </div>
          ...
          <div key={item.id} className="bg-white rounded-xl p-4 flex gap-3 items-start">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-2">
                <button onClick={() => updateQuantity(item.id, -1)} className="w-8 h-8 rounded-full bg-[#C8102E] text-white flex items-center justify-center text-lg font-light">−</button>
                <span className="font-bold text-gray-900">{item.quantity}</span>
                <button onClick={() => updateQuantity(item.id, 1)} className="w-8 h-8 rounded-full bg-[#C8102E] text-white flex items-center justify-center text-lg font-light">+</button>
              </div>
            </div>
            <button onClick={() => removeFromCart(item.id)} className="text-gray-300 hover:text-gray-500 mt-1">
      ...
      <div className="mx-4 mt-4 bg-white rounded-xl p-4">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-gray-500 text-xs mb-1">{t('deliveryAddress')}</div>
            <div className="font-semibold text-gray-900 text-sm">{address?.street ?? t('notSelected')}</div>
          </div>
          <button onClick={() => navigate('/map')} className="text-[#C8102E] text-sm font-semibold">
            {address ? t('change') : t('add')}
          </button>
        </div>
      </div>
      ...
      >
        ...
        <button onClick={handleOrder} className="w-full bg-[#C8102E] text-white py-4 rounded-xl font-bold text-base">
          {t('checkout')}
        </button>
      </div>
    </div>
